...
class TelaCadastroMembros(tk.Toplevel):
    ...
    def _construir_ui(self):
        ...
        self.btn_cadastrar = tk.Button(
            self.canvas, text="Cadastrar Membro", font=("Segoe UI Semibold", 12),
    ...
    def _ao_redimensionar(self, evento=None):
        ...
        self.canvas.create_text(cx, cy - 165, text="Cadastro de Membros", font=("Segoe UI Light", 12), fill="#ffffff")
    ...
    def _cadastrar(self):
        ...
        if not nome:
            self._notificacao("Informe o nome do membro.")
            return
        if not email:
            self._notificacao("Informe o e-mail do membro.")
            return
        if not cpf:
            self._notificacao("Informe o CPF do membro.")
            return
    ...
    def _salvar(self, nome, email, telefone, cpf, sala, turno):
        ...
        if sucesso:
            self._notificacao("Membro cadastrado com sucesso!")
            self._limpar_campos()
        else:
            self._notificacao("Erro ao cadastrar membro.")
...
if __name__ == "__main__":
    root = tk.Tk()
    root.withdraw()
    app = TelaCadastroMembros(master=root)
    app.mainloop()
    root.destroy()
